...
function Dock({
  ...
  return (
    <motion.div
      style={
        isVertical
          ? { width: height, scrollbarWidth: 'none' }
          : { height: height, scrollbarWidth: 'none' }
      }
      className={
        isVertical
    ...
    >
      <motion.div
        ...
        className={cn(
          ...
          isVertical && 'flex-col py-4 px-0 items-center',
          className
        )}
      ...
      >
        <DockProvider
          value={{ mouseX, spring, distance, magnification, direction }}
        >
          {children}
        </DockProvider>
      </motion.div>
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            'absolute z-50 w-fit whitespace-pre rounded-md border border-gray-200 bg-gray-100 px-2 py-0.5 text-xs text-neutral-700 dark:border-neutral-900 dark:bg-neutral-800 dark:text-white',
            isVertical
